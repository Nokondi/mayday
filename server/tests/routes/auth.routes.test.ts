...
const mockedSendRegistrationCollisionEmail = vi.mocked(
  sendRegistrationCollisionEmail,
);
...
describe("POST /api/auth/register", () => {
  it("creates an unverified user and dispatches a verification email", async () => {
    ...
    mockedUser.create.mockImplementationOnce(
    ...
    const createArgs = mockedUser.create.mock.calls[0][0] as {
    ...
    const [dispatchedTo, dispatchedToken] =
      mockedSendVerificationEmail.mock.calls[0];
    ...
    expect(verifyVerificationToken(dispatchedToken as string)).toEqual({
      userId: "u1",
    });
  });
  ...
  it("returns the same generic response when the email is already registered (no enumeration)", async () => {
    ...
    expect(mockedSendRegistrationCollisionEmail).toHaveBeenCalledWith(
      "alice@example.com",
    );
  });
...
describe("POST /api/auth/forgot-password", () => {
  it("dispatches a reset email for a matching account", async () => {
    ...
    expect(decoded).toEqual({
      userId: "u1",
      pv: passwordFingerprint("hashed-password-abc"),
    });
  });
...
describe("POST /api/auth/reset-password", () => {
  it("updates the password when the token and fingerprint are valid", async () => {
    ...
    const updateArgs = mockedUser.update.mock.calls[0][0] as {
  ...
  it("rejects when the password hash fingerprint no longer matches (e.g. link already used)", async () => {
    ...
    const staleToken = signPasswordResetToken({
      id: "u1",
