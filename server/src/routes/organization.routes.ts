...
organizationRoutes.delete(
  ...
  asyncHandler(async (req: AuthRequest, res) => {
    ...
    if (!isSelf) {
      ...
      throw new AppError(
        400,
        "Owners cannot leave; transfer ownership or delete the organization",
      );
    }
...
organizationRoutes.post(
  "/:id/invites",
  validate(inviteToOrganizationSchema),
  asyncHandler(async (req: AuthRequest, res) => {
    const orgId = req.params.id as string;
    const membership = await prisma.organizationMember.findUnique({
      where: {
    ...
    if (!targetUser) {
      ...
      try {
        ...
        console.error(
          "[mail] failed to send organization-signup-invite email",
          err,
        );
      }
    ...
    const existingMember = await prisma.organizationMember.findUnique({
      where: {
        organizationId_userId: { organizationId: orgId, userId: targetUser.id },
      },
    });
