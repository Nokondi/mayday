...
import { createOrganization, inviteToOrganization } from '../api/organizations.js';
...
export function CreateOrganizationPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (org) => {
      ...
      const emails = Array.from(new Set(inviteEmails.map((e) => e.trim()).filter(Boolean)));
      if (emails.length > 0) {
        ...
        if (ok && !failed) {
          toast.success(`Organization created · ${ok} invite${ok === 1 ? '' : 's'} sent`);
        } else if (ok && failed) {
          toast.success(`Organization created · ${ok} invited, ${failed} failed`);
        } else {
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        >
          ...
          <div>
            <label htmlFor="org-description" className="block text-sm font-medium text-gray-700 mb-1">Description</label>
            <textarea
              id="org-description"
          ...
          <div>
            <label htmlFor="org-location" className="block text-sm font-medium text-gray-700 mb-1">Location</label>
            <input
              id="org-location"
              {...register('location')}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-mayday-500 focus:border-transparent"
          ...
          <div>
            <label htmlFor="org-avatar-url" className="block text-sm font-medium text-gray-700 mb-1">Avatar URL <span className="text-gray-500 font-normal">(optional)</span></label>
            <input
              id="org-avatar-url"
              {...register('avatarUrl')}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-mayday-500 focus:border-transparent"
              ...
              {inviteEmails.map((email, idx) => (
                <div key={idx} className="flex gap-2">
                  <input
                    ...
                    value={email}
                    onChange={(e) =>
                      setInviteEmails((prev) => prev.map((v, i) => (i === idx ? e.target.value : v)))
                    }
