...
import { formatRecurrence, statusLabels, PostCard } from "./PostCard.js";
import { PostTypeBadge } from "../common/PostTypeBadge.js";
import { getPostMatches, deletePost, reopenPost } from "../../api/posts.js";
...
type Section = "comments" | "related";

/**
 * The full post-detail UI: header, images, badges, meta, action buttons, and
 * the comments / matching-posts sections, plus the fulfill and report dialogs.
 *
 * Two variants share one implementation:
 * - "page" (PostDetailPage): the body gets its own card, the sections render
 *   below it as tabs with Comments active by default.
 * - "card" (expanded PostCard in a list): the caller provides the card shell,
 *   both sections start collapsed, and each section button toggles open/closed
 *   so the card only grows when the viewer asks for more.
 */
export function PostDetailContent({
  ...
  post: PostWithAuthor;
  variant: "page" | "card";
  /** Card variant only: renders a collapse control that calls this. */
  onCollapse?: () => void;
  /** Called after a successful delete instead of any built-in navigation. */
  onDeleted?: () => void;
}) {
  const intl = useIntl();
  const { user } = useAuth();
  ...
  // Events and comms have no opposite type to match against, so the section
  // is hidden and the fetch never runs for them.
  const hasMatches = post.type === "REQUEST" || post.type === "OFFER";

  // Matches are only fetched once the viewer opens the section.
  const { data: matches } = useQuery({
    queryKey: ["postMatches", post.id],
    queryFn: () => getPostMatches(post.id),
    enabled: !!user && hasMatches && activeSection === "related",
  });
  ...
  const showActionCluster = !!user || !!onCollapse;
  ...
  const sectionButtonClass = (section: Section) =>
    ...
    <div className={variant === "page" ? "mt-8" : "mt-6"}>
      ...
      >
        <button
          ...
          aria-selected={
            variant === "page" ? activeSection === "comments" : undefined
          }
          aria-expanded={
            variant === "card" ? activeSection === "comments" : undefined
          }
          onClick={() => toggleSection("comments")}
          className={sectionButtonClass("comments")}
        >
          <FormattedMessage
        ...
        {hasMatches && (
          <button
            ...
            role={variant === "page" ? "tab" : undefined}
            aria-selected={
              variant === "page" ? activeSection === "related" : undefined
            }
            aria-expanded={
              variant === "card" ? activeSection === "related" : undefined
            }
            onClick={() => toggleSection("related")}
            className={sectionButtonClass("related")}
          >
            {post.type === "REQUEST" ? (
              <FormattedMessage
  ...
  return (
    <>
      <div
        className={
          variant === "page"
            ? "relative bg-white rounded-lg border border-mayday-200 p-6"
            : "relative"
        }
      >
        {showActionCluster && (
          <div className="absolute top-3 right-3 flex items-center gap-1">
            ...
            {onCollapse && (
              <button
                ...
                aria-expanded={true}
                aria-label={intl.formatMessage({
                  id: "posts.card.collapseAria",
                  defaultMessage: "Collapse post",
                })}
                title={intl.formatMessage({
                  id: "posts.card.collapseAria",
                  defaultMessage: "Collapse post",
        ...
        <div
          className={`flex items-start gap-3 mb-3 ${showActionCluster ? "pr-28" : ""}`}
        >
          <Link
      ...
      <dialog
        ref={reportDialogRef}
        aria-labelledby={`report-confirm-title-${post.id}`}
        className="rounded-lg p-0 backdrop:bg-black/50 max-w-md w-full"
      >
        <div className="p-6">
          <h2
            id={`report-confirm-title-${post.id}`}
          ...
          <div className="mt-4">
            <label
              htmlFor={`report-post-details-${post.id}`}
            ...
            <textarea
              id={`report-post-details-${post.id}`}
              value={reportDetails}
